import streamlit as st
import requests
import firebase_admin
from firebase_admin import credentials, firestore
from datetime import datetime
import uuid
import random

# --- 1. FIREBASE SETUP ---
db = None
if not firebase_admin._apps:
    try:
        s = st.secrets["firebase"]
        pk = s["private_key"]
        fb_creds = {
            "type": "service_account", "project_id": s["project_id"],
            "private_key_id": "eecd76124b0bb41c6c43d72db01c47203a29cc7d",
            "private_key": pk, "client_email": s["client_email"],
            "client_id": "110901490489199893217", "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
            "client_x509_cert_url": f"https://www.googleapis.com/robot/v1/metadata/x509/{s['client_email'].replace('@', '%40')}"
        }
        cred = credentials.Certificate(fb_creds)
        firebase_admin.initialize_app(cred)
        db = firestore.client()
    except: pass

if firebase_admin._apps and not db:
    try: db = firestore.client()
    except: pass

# --- 2. CONFIG ---
st.set_page_config(page_title="NexusAI Global", page_icon="🌐", layout="wide")

if "chat_id" not in st.session_state:
    st.session_state.chat_id = str(uuid.uuid4())[:8]

# --- 3. SIDEBAR (Everything in English) ---
with st.sidebar:
    st.title("🌐 NexusAI Navigation")
    
    if st.button("➕ New Chat", use_container_width=True):
        st.session_state.chat_id = str(uuid.uuid4())[:8]
        st.rerun()
    
    st.divider()
    if db:
        st.subheader("Chat History")
        try:
            history = db.collection("nexus_chats").document("petar").collection("sessions").order_by("start_time", direction=firestore.Query.DESCENDING).limit(10).stream()
            for h in history:
                title = h.to_dict().get("first_msg", "New Chat")[:20]
                if st.button(f"💬 {title}", key=h.id):
                    st.session_state.chat_id = h.id
                    st.rerun()
        except: 
            st.write("History currently unavailable.")

# --- 4. MAIN CHAT LOGIC ---
st.title(f"🌐 NexusAI Console")

chat_context = "" 

if db:
    try:
        msgs_ref = db.collection("nexus_chats").document("petar").collection("sessions").document(st.session_state.chat_id).collection("messages").order_by("timestamp")
        all_msgs = list(msgs_ref.stream())
        
        for i, m_doc in enumerate(all_msgs):
            m = m_doc.to_dict()
            with st.chat_message(m["role"]):
                if "https://" in m["text"]: st.image(m["text"])
                else: st.write(m["text"])
            
            if "https://" not in m["text"] and i > len(all_msgs) - 6:
                chat_context += f"{m['role']}: {m['text']}\n"
    except: pass

# Input in English
prompt = st.chat_input("Message NexusAI...")

if prompt:
    st.chat_message("user").write(prompt)
    
    if db:
        try:
            db.collection("nexus_chats").document("petar").collection("sessions").document(st.session_state.chat_id).set({"first_msg": prompt, "start_time": datetime.now()}, merge=True)
            db.collection("nexus_chats").document("petar").collection("sessions").document(st.session_state.chat_id).collection("messages").add({"role":"user", "text": prompt, "timestamp": datetime.now()})
        except: pass

    with st.chat_message("assistant"):
        if any(w in prompt.lower() for w in ["draw", "image", "generate photo", "picture"]):
            with st.spinner("Nexus is generating your image..."):
                seed = random.randint(0, 99999)
                url = f"https://image.pollinations.ai/prompt/{prompt.replace(' ', '%20')}?width=1024&height=1024&model=flux&seed={seed}"
                st.image(url)
                ans = url
        else:
            # AI is instructed to stay in English and maintain context
            sys_instr = f"Your name is NexusAI. Respond only in English. Maintain the conversation context and be helpful. Context:\n{chat_context}"
            try:
                res = requests.get(f"https://text.pollinations.ai/{sys_instr} User: {prompt}?model=openai")
                st.write(res.text)
                ans = res.text
            except:
                st.error("Nexus connection lost.")
                ans = "Error."
        
        if db:
            try: db.collection("nexus_chats").document("petar").collection("sessions").document(st.session_state.chat_id).collection("messages").add({"role":"assistant", "text": ans, "timestamp": datetime.now()})
            except: pass
